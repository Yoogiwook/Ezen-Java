...
public class Calendar {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		int[] days_per_month = new int[] {31, 28, 31, 30, 31, 30, 31, 31,
											30, 31, 30, 31};// 각 달의 마지막 날을 배열로 지정
		
		Exit : {
			User : while (true) {
				System.out.println("▼ 연도를 입력해주세요! (종료를 원하시면 0을 입력해주세요.)");
				year = sc.nextInt();
				if (year < 0) { // 0보다 작은값을 입력하면 실행x 다시 입력 
					System.out.println("아쉽게도 기원전 달력 보기는 지원하지 않아요. 다시...");
					Thread.sleep(500);
					continue;
				}
				else if (year == 0) { //0을 입력하면 종료
				...
				while (true) {
					...
					month = sc.nextInt();
					if(month < 1 || month > 12) { // 1~12 정수만 입력가능 잘못 입력시 다시입력
